...
function Todos() {
  ...
  const addItem = async (e) => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:5500/api/user/${userId}/item`,
        { item: itemText }
      );
  ...
  // Create function to fetch all todo items for a user from database -- we will use useEffect hook
  useEffect(() => {
    const getItemsList = async () => {
      try {
        const res = await axios.get(
          `http://localhost:5500/api/user/${userId}/items`
        );
...
<button className="delete-item" onClick={logout}>
Logout
</button>
</div>
);
